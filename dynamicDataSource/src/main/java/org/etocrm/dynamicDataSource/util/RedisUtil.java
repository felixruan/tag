...
public class RedisUtil {
    ...
    public <T> T getOauthTokenRefresh(String key, Class<T> clazz) {
        key = key.substring(7, key.length());
        T t = get(key, clazz, -1L);
        set(key, t, RedisConfig.expire);
        return t;
    }

    public <T> T getOauthToken(String key, Class<T> clazz) {
        key = key.substring(7, key.length());
        T t = get(key, clazz, -1L);
        return t;
    }

    public <T> T getRefresh(String key, Class<T> clazz) {
        T t = get(key, clazz, -1L);
        set(key, t, RedisConfig.expire);
        return t;
    }
